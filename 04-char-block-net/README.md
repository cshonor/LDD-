...
### 0.2 /dev 本身是什么：devtmpfs（内存文件系统）

Pi 5 实测（内核 6.18.39）：

```
$ mount | grep " on /dev "
udev on /dev type devtmpfs (rw,nosuid,relatime,size=4085024k,nr_inodes=255314,mode=755)

$ sudo touch /dev/hello-test && ls -l /dev/hello-test
-rw-r--r-- 1 root root 0 ... /dev/hello-test      ← 能建普通文件！
```

要点：

- `/dev` 是开机时内核自动挂载的 **devtmpfs**——纯内存文件系统，
  不是 EMMC/SD 上的目录。断电/重启后节点全消失，内核开机重新生成。
- 铁证：在 `/dev` 里能 `touch` 出一个**普通文件**（实测通过）——
  它就是个真文件系统，设备节点只是其中一种特殊文件（type 为 char/block 的 inode）。
- **门牌与硬件分开记**：节点（文件名/inode/主次号）在内存里；
  节点背后的数据各回各家——`mmcblk0` 的数据写进 EMMC 闪存（断电保留），
  tty/fb/声卡没有存储，只是硬件通道。
- 两处修正（对照"udev 已被取代"的常见说法）：
  1. udev 并没有退场，而是**搬到 devtmpfs 之上**干活——挂载输出里
     源名就叫 `udev`、类型才是 `devtmpfs`：内核负责**创建节点**，
     udev（用户态）负责改名/改权限/建符号链接（如 `eth0` 的改名）。
     只有 busybox 级别的迷你嵌入式才用 mdev 替代 udev。
  2. "字符设备直接和硬件寄存器交互"过粗：tty 要过线路规程缓冲、
     input 走 evdev 队列——准确说法是**不走块层和页缓存**，路径更短，
     但中间仍有内核缓冲。
- 顺带区分经常一起出现的三个内存文件系统：

| 挂载点 | 类型 | 谁提供 | 内容 |
|--------|------|--------|------|
| `/dev` | devtmpfs | 内核（驱动注册时） | 设备节点（inode 带 major:minor） |
| `/sys` | sysfs | 内核统一设备模型 | 设备/驱动/总线的**属性文件**（`cat /sys/class/...` 可读写参数） |
| `/proc` | procfs | 内核各子系统 | 进程/内核状态的**窗口**（`/proc/devices`、`/proc/interrupts`） |

共同点：都在内存、断电即逝、读写文件 = 与内核对话；
区别：`/dev` 给**数据通道**，`/sys` 给**设备属性**，`/proc` 给**进程与内核状态**。
